...
import com.bsisoftware.mhu.ants.shared.exception.RemoteAntsException;
...
public class AntsClient {
	...
	public ConfigurationsDTO getConfigs() throws RemoteAntsException {
		Invocation invocation = target.
			path(UriUtil.create(UriUtil.PATH_CONFIGS)).
			request(MediaType.APPLICATION_JSON_TYPE).
			accept(MediaType.APPLICATION_JSON_TYPE).buildGet();
		return doRemoteCall(invocation, ConfigurationsDTO.class);
	}

	private ConfigurationsDTO doRemoteCall(Invocation invocation, Class<ConfigurationsDTO> clazz) throws RemoteAntsException {
